...
            var t = $(this).combotree('tree');
			var opt = {me:$(this), searchText:q};
            t.tree("search",opt);
        }
    });
	$.extend($.fn.combotree.defaults,{
		customFilters:{
			isFilter: false,//是否进行数据过滤
			filterFields:["text"] //要过滤的字段
		}
	});
...
        search: function (jqTree,opt) {
			var me = opt.me;
			var searchText = opt.searchText;
			
			var isFilter = me.combotree('options').customFilters.isFilter;
			if(!isFilter){
				return;
			}
			var filterFields = me.combotree('options').customFilters.filterFields;
...
                    node = nodeList[i];
					...
					for(var j=0; j < filterFields.length; j++ ){
						if (isMatch(searchText, node[filterFields[j]])) {
